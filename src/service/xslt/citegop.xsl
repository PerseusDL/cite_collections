...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    version="1.0" xmlns:cite="http://chs.harvard.edu/xmlns/cite"  xmlns:exsl="http://exslt.org/common">
    ...
    <!-- this is a hack until create_from_linked_urn supports new transcriptions -->
...
-->                
    ...
    <xsl:template match="cite:urn">
        <xsl:variable name="coll"><xsl:value-of select="substring-before(substring-after(substring-after(.,'urn:cite:'),':'),'.')"/></xsl:variable>
        ...
        <a href="{concat($CollectionEditorUrl,$coll,'&amp;',$prop,'=',.)}"><xsl:value-of select="."/></a>
    </xsl:template>
